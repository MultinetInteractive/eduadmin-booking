...
if ( wp_verify_nonce( $_POST['edu-interest-nonce'], 'edu-object-interest' ) ) {
	...
	if ( $missing_fields ) {
		...
		$inquiry->CompanyName = wp_unslash( sanitize_text_field( $_POST['edu-companyName'] ) ); // Input var okay
		$inquiry->FirstName   = wp_unslash( sanitize_text_field( $_POST['edu-contactFirstName'] ) ); // Input var okay
		$inquiry->LastName    = wp_unslash( sanitize_text_field( $_POST['edu-contactLastName'] ) ); // Input var okay
		$inquiry->Email       = wp_unslash( sanitize_email( $_POST['edu-emailAddress'] ) ); // Input var okay
		$inquiry->Phone       = wp_unslash( sanitize_text_field( $_POST['edu-phone'] ) ); // Input var okay
		$inquiry->Mobile      = wp_unslash( sanitize_text_field( $_POST['edu-mobile'] ) ); // Input var okay
		$inquiry->Notes       = wp_unslash( sanitize_textarea_field( $_POST['edu-notes'] ) ); // Input var okay
