...
function edu_write_certificate_date( $start_date, $end_date ) {
	...
	if ( ! empty( $start_date ) ) {
		$date_str = edu_get_timezoned_date( 'Y-m-d', $start_date ) . ' - ';
	}

	if ( ! empty( $end_date ) ) {
		$date_str .= edu_get_timezoned_date( 'Y-m-d', $end_date );
	} else {
				...
				if ( ! empty( $certificateHolder['Certificates'] ) ) {
					foreach ( $certificateHolder['Certificates'] as $certificate ) {
						...
						<tr>
							...
							<td align="left"><?php echo esc_html( edu_get_timezoned_date( 'Y-m-d', $certificate['CertificateDate'] ) ); ?></td>
