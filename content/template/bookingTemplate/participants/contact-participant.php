<div id="contactPersonParticipant" class="participantItem contactPerson" style="display: none;">
	...
	<?php if ( 'selectParticipant' === EDU()->get_option( 'eduadmin-selectPricename', 'firstPublic' ) ) { ?>
		<label class="edu-book-contactparticipant-contactPriceName">
			...
			<div class="inputHolder">
				<select name="contactPriceName" autocomplete="off"
				        class="edudropdown participantPriceName edu-pricename" data-required="true"
	...
	if ( ! empty( $event['Sessions'] ) ) {
		...
		foreach ( $event['Sessions'] as $sub_event ) {
			...
			echo( intval( $s ) > 0 ? '&nbsp;<i class="priceLabel">' . esc_html( edu_get_price( $s, $event['ParticipantVat'] ) ) . '</i>' : '' );
			echo "</label>\n";
		}
		echo '<br />';
	}
