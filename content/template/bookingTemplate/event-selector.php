	...
	<select name="eid" required class="dateInfo" onchange="eduBookingView.SelectEvent(this);">
		...
		<?php foreach ( $events as $ev ) : ?>
			<option value="<?php echo esc_attr( $ev['EventId'] ); ?>">
				...
				echo esc_html( date_i18n( 'H:i', strtotime( $ev['StartDate'] ) ) );
				?>
				-
				<?php
				echo esc_html( date_i18n( 'H:i', strtotime( $ev['EndDate'] ) ) );
...
elseif ( 1 === count( $events ) ) :
	...
	-
	<?php
	echo esc_html( date_i18n( 'H:i', strtotime( $event['EndDate'] ) ) ) . '</span>';
